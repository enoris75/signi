...
// Regression: an owner's builder that remounts with pre-existing content into a cramped canvas
// used to drive the workspace geometry-notification effect into an unbounded bump→render→bump loop
// (Maximum update depth exceeded, crashing the canvas). The `page` fixture fails the test on any
// uncaught page error, which is what catches the loop.
//
// The crash needs a narrow viewport so the rings have to be shoved to settle, and it needs the
// owner's ring to be *reopened* (mounting with its genitive head already filled) rather than filled
// in place.
...
async function fillPossessor(page: import('@playwright/test').Page) {
  ...
  await page.getByTestId('typeahead-noun').fill('light');
...
test('reopening a filled manner possessor in a cramped canvas does not loop', async ({
  ...
  await togglePossessor(page); // reopen — the owner's builder remounts with LIGHT already present
