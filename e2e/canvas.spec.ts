...
test.describe('canvas', () => {
  ...
  test("a word's controls ride its solid ring, clear of its word and of each other", async ({ app }) => {
    ...
    for (const [slot, word, controls] of [
      ...
      // and so do the question mark, its who / what chip and the existential (P09-E12 D7, D8), and the
      // noun's standard and examples controls in its relations fan (P09-E50, P09-E48).
      [
        'subject',
        'Africa',
        `${['Relative', 'Possessor', 'Conjunct', 'Question', 'QuestionAnimate', 'Existential', 'Standard', 'Examples'].reduce(
