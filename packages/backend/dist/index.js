...
app.get('/api/concepts', (req, res) => {
    ...
    if (role) {
        rows = db
            .prepare('SELECT id, role, description, emoji, transitivity FROM semantic_concepts WHERE role = ? ORDER BY id')
            .all(role);
    }
    else {
        rows = db
            .prepare('SELECT id, role, description, emoji, transitivity FROM semantic_concepts ORDER BY role, id')
            .all();
    }
    const labelRows = db.prepare(EN_LABEL_SQL).all();
    const labels = new Map(labelRows.map((r) => [r.concept_id, r.en_word]));
    const genderedNounRows = db.prepare(GENDERED_NOUNS_SQL).all();
    const genderedNouns = new Set(genderedNounRows.map((r) => r.concept_id));
    const pronounMeta = db
        .prepare(PRONOUN_META_SQL)
        .all();
    const pronounPersons = new Map(pronounMeta.map((r) => [r.concept_id, r.person]));
    const pronounNumbers = new Map(pronounMeta.map((r) => [r.concept_id, r.number]));
    const response = {
        concepts: rows.map((r) => ({
            id: r.id,
            role: r.role,
            ...
            transitivity: r.transitivity ?? undefined,
