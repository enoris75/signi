...
const concepts = [
    ...
    },
    {
        ...
        emoji: '🚫',
        forms: {
            en: { base: 'never', subtype: 'frequency' },
            it: { base: 'mai', subtype: 'frequency' },
            fr: { base: 'jamais', subtype: 'frequency' },
            de: { base: 'nie', subtype: 'frequency' },
            es: { base: 'nunca', subtype: 'frequency' },
            ja: { base: '決して', subtype: 'frequency' },
            pt: { base: 'nunca', subtype: 'frequency' },
        },
    },
];
function buildRoleStmts(role) {
    return {
        insertLexeme: db.prepare(roleInsertSql(role)),
        insertForm: db.prepare(`INSERT INTO ${role}_forms (lexeme_id, form_key, form_value) VALUES (?, ?, ?)`),
        insertLink: db.prepare(`INSERT INTO concept_${role}_links (concept_id, lexeme_id, is_primary) VALUES (?, ?, 1)`),
    };
}
function roleInsertSql(role) {
    switch (role) {
...
function lexemeArgs(role, lang, lemma, forms) {
    switch (role) {
        case 'noun':
...
function seed() {
    const stmts = {
        ...
        insertConcept: db.prepare('INSERT INTO semantic_concepts (id, role, description, emoji, transitivity) VALUES (?, ?, ?, ?, ?)'),
    };
    const roleStmts = {
