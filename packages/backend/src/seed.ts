...
function seed() {
  const stmts = {
    ...
    insertConcept: db.prepare<[string, string, string, string | null, string | null, string | null, number, number, string | null, number, number, number, string | null, string | null, number, number, number, number, string | null]>(
      'INSERT INTO semantic_concepts (id, role, description, emoji, transitivity, complements, animate, human, synonym, countable, modal, proper, manner_relation, dimension_relation, transient, alarm, alarm_cry, stative, sense_of) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)'
    ),
  ...
  const run = db.transaction(() => {
    ...
    for (const c of concepts) {
      stmts.insertConcept.run(c.id, c.role, c.description, c.emoji ?? null, c.transitivity ?? null, c.complements?.length ? c.complements.join(',') : null, c.animate ? 1 : 0, c.human ? 1 : 0, c.synonym ?? null, c.countable === false ? 0 : 1, c.modal ? 1 : 0, c.proper ? 1 : 0, c.mannerRelation ?? null, c.dimensionRelation ?? null, c.transient ? 1 : 0, c.alarm ? 1 : 0, c.alarmCry ? 1 : 0, c.stative ? 1 : 0, c.senseOf ?? null);
