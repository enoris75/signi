...
const keyOf = (concept: string, f: PronominalPossessor) =>
  `${concept}:${f.person}${f.number}${f.gender ?? ""}`;
 ...
 *
 * One `/api/translate` call per distinct (noun, antecedent) pair, cached forever — the render
 * depends only on the plan and the lexicon, and the lexicon does not move while the app is open.
 * Until it answers the caller falls back to the catalog's bare `pronoun.possessive.*`, which is
 * right in three of the seven languages and never blank.
 */
export function usePossessivePhrases(requests: readonly PossessiveRequest[]): PossessivePhrase {
  ...
  const unique = [...new Map(requests.map((r) => [keyOf(r.concept, r.features), r])).entries()]
    .sort(([a], [b]) => (a < b ? -1 : a > b ? 1 : 0));

  const results = useQueries({
    queries: unique.map(([key, { concept, features }]) => ({
      queryKey: ["possessive-phrase", key],
      // A bare noun phrase, no verb: the possessive is the whole point, and each engine supplies
      // the article its language puts in front of one ("il suo libro", "o seu livro").
      queryFn: () => fetchTranslation({ subject: { concept, possessor: features } }),
      staleTime: Infinity,
    })),
  });
  ...
  return (concept, features) =>
    concept && features ? byKey.get(keyOf(concept, features)) : undefined;
}
