...
function resolve(conceptId, language, lookup) {
    const entry = lookup(conceptId, language);
    return { conceptId, forms: entry ? { ...entry.forms } : {} };
}
function applyNounGender(forms, gender) {
    if (gender !== 'fem' || !forms['fem'])
        return;
    const plural = forms['number'] === 'plural';
    forms['base'] = plural ? (forms['fem_plural'] ?? forms['fem']) : forms['fem'];
    if (plural && forms['fem_plural'])
        forms['plural'] = forms['fem_plural'];
    forms['gender'] = 'fem';
}
export function translate(plan, lookup) {
    return engines.map((engine) => {
        ...
        if (subjectForms.forms['person']) {
            ...
            if (number === 'plural') {
                if (subjectForms.forms['plural'])
                    subjectForms.forms['base'] = subjectForms.forms['plural'];
            }
            else if (subjectForms.forms['person'] === '3') {
                const gf = subjectForms.forms[`singular_${gender}`];
                if (gf)
                    subjectForms.forms['base'] = gf;
