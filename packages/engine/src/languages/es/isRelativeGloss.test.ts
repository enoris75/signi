...
import { COMER, el, LIBRO, np, vp } from './es.fixtures.js';
...
describe('isRelativeGloss', () => {
  test('is true for a single conjunct flagged as a relative gloss that carries a relative', () => {
    expect(isRelativeGloss(el(np(LIBRO, {}, { relative, relativeGloss: true })))).toBe(true);
  });

  test('is false when the flag is absent, false, or only another gloss', () => {
    expect(isRelativeGloss(el(np(LIBRO, {}, { relative })))).toBe(false);
    expect(isRelativeGloss(el(np(LIBRO, {}, { relative, relativeGloss: false })))).toBe(false);
    expect(isRelativeGloss(el(np(LIBRO, {}, { relative, mannerGloss: true })))).toBe(false);
  });
  ...
  test('is false for a flagged phrase with no relative', () => {
    expect(isRelativeGloss(el(np(LIBRO, {}, { relativeGloss: true })))).toBe(false);
  });

  test('is false for a coordination, even of flagged conjuncts', () => {
    const flagged = np(LIBRO, {}, { relative, relativeGloss: true });
