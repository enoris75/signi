 ...
 * ("creador de frases semánticas"), postnominal as in Spanish. A `domain` takes the generic
 * definite article, "del" contracted ("mosca de la fruta", "moscas del tiempo").
 */
export function modifierText(np: ResolvedNounPhrase): string {
  return np.nounModifiers
    .map((m) => {
      ...
      return ` ${m.relation === 'domain' ? dePrep(forms, plural) : REL_PREP_ES[m.relation]} ${nounPart}`;
    })
    .join('');
}
