...
import { complement, el, type Forms, CASA, np } from './pt.fixtures.js';
...
describe('complementGloss', () => {
  test('a locative takes its default "em", and the phrase keeps its own determiner', () => {
    expect(complementGloss(gloss({ definiteness: 'all', number: 'plural' }, 'locative'))).toBe('em todas as casas');
  });

  test('a direction with no relation is the plain goal', () => {
    expect(complementGloss(gloss({ definiteness: 'indefinite' }, 'direction'))).toBe('a uma casa');
    expect(complementGloss(gloss({}, 'direction'))).toBe('à casa');
  });

  test('a relation is the complement\'s own: a place under it, a goal "em" it, contracted with the article', () => {
    expect(complementGloss(gloss({}, 'locative', UNDER))).toBe('debaixo da casa');
    expect(complementGloss(gloss({}, 'direction', INTO))).toBe('na casa');
  });
