 ...
 * predicate ("j'ai mangé", "j'aime") and before the clitic "y" ("j'y suis"); any other subject, a
 * tonic "moi" included, takes a space.
 * Either side may be empty (an imperative has no subject).
 */
export function joinSubject(subject: string, predicate: string): string {
  if (subject === 'je' && (VOWEL_START.test(predicate) || /^y /.test(predicate))) return `j'${predicate}`;
