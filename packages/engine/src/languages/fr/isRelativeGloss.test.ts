...
import { el, LIVRE, MANGER, np, vp } from './fr.fixtures.js';
import { isRelativeGloss } from './isRelativeGloss.js';

const relative = { headRole: 'directObject' as const, verbPhrase: vp(MANGER) };

describe('isRelativeGloss', () => {
  test('is true for a single conjunct flagged as a relative gloss that carries a relative', () => {
    expect(isRelativeGloss(el(np(LIVRE, {}, { relative, relativeGloss: true })))).toBe(true);
  });

  test('is false when the flag is absent, false, or only another gloss', () => {
    expect(isRelativeGloss(el(np(LIVRE, {}, { relative })))).toBe(false);
    expect(isRelativeGloss(el(np(LIVRE, {}, { relative, relativeGloss: false })))).toBe(false);
    expect(isRelativeGloss(el(np(LIVRE, {}, { relative, mannerGloss: true })))).toBe(false);
  });
  ...
  test('is false for a flagged phrase with no relative', () => {
    expect(isRelativeGloss(el(np(LIVRE, {}, { relativeGloss: true })))).toBe(false);
  });

  test('is false for a coordination, even of flagged conjuncts', () => {
    const flagged = np(LIVRE, {}, { relative, relativeGloss: true });
