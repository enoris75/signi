...
import { EAT, el, np, OBJECT, vp } from './en.fixtures.js';
import { isRelativeGloss } from './isRelativeGloss.js';

const relative = { headRole: 'directObject' as const, verbPhrase: vp(EAT) };

describe('isRelativeGloss', () => {
  test('is true for a single conjunct flagged as a relative gloss that carries a relative', () => {
    expect(isRelativeGloss(el(np(OBJECT, {}, { relative, relativeGloss: true })))).toBe(true);
  });

  test('is false when the flag is absent, false, or only another gloss', () => {
    expect(isRelativeGloss(el(np(OBJECT, {}, { relative })))).toBe(false);
    expect(isRelativeGloss(el(np(OBJECT, {}, { relative, relativeGloss: false })))).toBe(false);
    expect(isRelativeGloss(el(np(OBJECT, {}, { relative, mannerGloss: true })))).toBe(false);
  });
  ...
  test('is false for a flagged phrase with no relative', () => {
    expect(isRelativeGloss(el(np(OBJECT, {}, { relativeGloss: true })))).toBe(false);
  });

  test('is false for a coordination, even of flagged conjuncts', () => {
    const flagged = np(OBJECT, {}, { relative, relativeGloss: true });
