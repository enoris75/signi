...
export function npText(np: ResolvedNounPhrase): string {
  return withRelative(nounPhrase(np.head.forms, npAdj(np), nounMods(np), np.possessor, npHasSuperlative(np), hasPartitivePossessor(np), npStandard(np), ownDetaches(np)), np);
}
