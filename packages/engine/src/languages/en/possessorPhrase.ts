...
export function possessorPhrase(poss: ResolvedNounPhrase): string {
  return withRelative(nounPhrase(poss.head.forms, npAdj(poss), nounMods(poss), poss.possessor, npHasSuperlative(poss), hasPartitivePossessor(poss), npStandard(poss)), poss);
}
