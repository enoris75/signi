...
export function inflect(base: string, suffix: 'er' | 'est'): string {
  ...
  // A stressed consonant-vowel-consonant ending doubles its final consonant: big → bigger. Only a
  // one-syllable base is stressed there; "clever" → "cleverer", not "cleverrer".
  if (syllables(base) === 1 && /[^aeiou][aeiou][^aeiouwxy]$/.test(base)) return base + base.slice(-1) + suffix;
  return base + suffix;
}
