...
/** Where the copula predicate stands, which picks its ending. */
export type CopulaForm = 'polite' | 'prenominal' | 'tara';

// The endings by class and form, as [affirmative present, affirmative past, negative present, negative
// past]. A たら form ignores tense, so it repeats its two cells.
 ...
 * "if" clause (`tara`: 大きかったら / 幸せだったら / 疲れていたら), which carries no tense.
 */
export function copulaSegs(pred: ResolvedComplement, tense: Tense, negative: boolean, form: CopulaForm = 'polite'): RubySegment[] {
  ...
  // A `no` noun predicate closes its circumfix in the copula: でもありません, not ではありません.
  if (f['role'] !== 'adjective') {
    const ending = COPULA_ENDINGS[form][cell];
    return [...elSegs(pred.phrase), { t: isNegativeGroup(pred.phrase) ? ending.replace(/^では/, 'でも') : ending }];
  }
