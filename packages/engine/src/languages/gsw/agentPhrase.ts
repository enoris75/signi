...
export function agentPhrase(agent?: ResolvedNounElement): string {
  if (!agent) return '';
  return coordinate(agent, (conjunct) => {
    // A definite owner that is a person or an animal stands in front as a possessor dative (P10-E12).
    const np = withPossessorDative(conjunct);
    if (np.head.forms['person']) {
      return `vo ${np.head.forms['disjunctive'] ?? np.head.forms['base'] ?? ''}${subordinateClause(np)}`;
    }
    ...
    const head = prepDet('vo', f, 'dat', plural);
    ...
    const possessive = poss
      ? `${possessiveGsw(poss, 'dat', { gender: (f['gender'] ?? 'neut') as 'masc' | 'fem' | 'neut', number: plural ? 'plural' : 'singular' })} `
      : '';
    const dativeOwner = possessorDative(np, 'dat');
    const rest = `${dativeOwner ? `${dativeOwner} ` : ''}${possessive}${adj}${word}${postnominal(f)}${modifierGenitives(np)}${possessorText(np)}${nounStandard(np, 'dat')}${subordinateClause(np)}${nounExamples(np, 'dat')}`;
