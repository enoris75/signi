...
import { BETWEEN_PREP } from '../gsw.consts.js';
...
import { CONSTITUENT_NEGATOR, DE_GENITIVE_TEMPORAL, DE_TEMPORAL, DIRECTION_IDIOMS, ESSIVE_ROLE_CASE, LOCATIVE_IDIOMS, OBJECT_PREDICATIVE_CASE } from '../gsw.consts.js';
import type { Case, ObjectPredicateHost } from '../gsw.types.js';
...
import { possessorDative, possessorText, withPossessorDative } from '../possessorText.js';
...
export function complementsParts(
  ...
  const render = (type: ComplementType): string => {
      ...
      // A definite owner that is a person or an animal stands in front as a possessor dative, in the
      // determiner's place after the preposition: "mit em Vatter sim Hund" (P10-E12).
      const np = essive ? withDefiniteness(conjunct, 'bare') : withPossessorDative(cardinalOne(conjunct));
      ...
      if (type === 'route' || type === 'locative') {
        ...
        else if (isPrivative(type, c)) { _case = 'acc'; head = prepDet('ohni', f, 'acc', plural); }
        ...
        // took `causePhrase` above.
          ...
          // No genitive (P10 D7): *wäge* and *dank* both take the dative, "wäge em Hund".
          _case = 'dat';
          head = prepDet(positive ? 'dank' : 'wäge', f, _case, plural);
        }
        ...
        // dative recipient, and keeps the recipient's place ahead of the object.
          ...
          else { _case = 'acc'; head = prepDet(f['place_prep'] ?? 'i', f, 'acc', plural); }
        }
        ...
        else /* source */         head = prepDet(f['animate'] === '1' || f['place_prep'] ? 'vo' : 'us', f, 'dat', plural);
      }
      ...
      const possessive = poss && !detached
        ? `${possessiveGsw(poss, nounCase, { gender: (f['gender'] ?? 'neut') as 'masc' | 'fem' | 'neut', number: plural ? 'plural' : 'singular' })} `
        : '';
      const vonPhrase = detached && poss ? ` vo ${dativePronounGsw(poss)}` : '';
      ...
      const rest = `${dativeOwner ? `${dativeOwner} ` : ''}${possessive}${counted}${adj}${word}${postnominal(f)}${modifierGenitives(np)}${vonPhrase}${possessorText(np)}${nounStandard(np, _case)}${subordinateClause(np)}${nounExamples(np, _case)}`;
  ...
  const predicate = text && nominal ? `${text} z sii` : text;
