...
import type { ObjectPredicateHost } from './gsw.types.js';
...
import { RELATIVE_WO, relativePronoun, resumptive } from './relativePronoun.js';
 ...
 */
/**
 * A possessor relative's possessed phrase with the possessive of its owner, the head: "wo **sis** Buech",
 * "wo **ires** Huus" — the possessive agreeing with the possessed noun, its owner the head's third person.
 */
function possessedWithOwner(head: Record<string, string>, possessed: Parameters<typeof subjectText>[0]): string {
  const text = subjectText(possessed);
  const agreement = possessed.agreement;
  const ownerPlural = (head['number'] ?? head['count']) === 'plural';
  const owner = { kind: 'pronominal' as const, person: '3' as const, number: ownerPlural ? 'plural' as const : 'singular' as const, gender: (head['gender'] ?? 'neut') as 'masc' | 'fem' | 'neut' };
  const word = possessiveGsw(owner, 'nom', {
    gender: (agreement['gender'] ?? 'neut') as 'masc' | 'fem' | 'neut',
    number: (agreement['number'] ?? agreement['count']) === 'plural' ? 'plural' : 'singular',
  });
...
export function subordinateClause(np: ResolvedNounPhrase): string {
  ...
  // Swiss German relativises with the invariant *wo* (P10 D10, E11): nothing is left behind for a
  // subject, a direct object or a predicate noun — "de Maa, wo chunt", "d Frau, wo ich gsee" — and a
  // dative, prepositional or agent role keeps a resumptive pronoun in the clause (see `resumptive`):
  // "de Maa, wo ich im s Buech gib", "s Huus, wo ich drin wohn". A possessor relative says whose with
  // the possessive: "de Maa, wo sis Buech …".
  const animate = f['animate'] === '1' || f['human'] === '1';
  const pronoun = possessed
    ? [RELATIVE_WO, possessedWithOwner(f, possessed)].filter(Boolean).join(' ')
    : RELATIVE_WO;
  const resumed = possessed
    ? ''
    : rel.headRole === 'agent'
      ? `vo ${relativePronoun(f, 'dat', plural)}`
    : gap
      ? resumptive(complementsPhrase(gap, rel.verbPhrase.verb.forms), animate)
      // A bare dative object (*hälfe*, `objectCase`) keeps its pronoun: "de Hund, wo me im hilft". A
      // preposition the verb takes its object with keeps it too: "d Taste, wo de Chater druf klickt".
      : !subjectRelative && rel.headRole !== 'predicative' && (headPrep || objectCase(rel.verbPhrase.verb) === 'dat')
        ? resumptive([headPrep, relativePronoun(f, headPrep ? objectPrepCase(headPrep) : 'dat', plural)].filter(Boolean).join(' '), animate)
        : '';
