...
export function renderClause(given: ResolvedPhrase, inverted = false, verbFinal = false, zu = false): string {
  ...
  const withContent = phrase.contentSubject
    ? `${clause}, das ${renderClause(phrase.contentSubject, false, true)}`
    : clause;
  ...
  const purposed = phrase.purpose
    // Zürich frames a purpose with *für … z* (Standard *um … zu*): "für d Übersetzige z gsee".
    ? `${governed}, für ${renderClause(phrase.purpose, false, false, true)}`
    : governed;
...
function clauseText(phrase: ResolvedPhrase, inverted: boolean, verbFinal: boolean, zu: boolean, bareGoverned = ''): string {
    ...
    if (mood === 'imperative') {
      ...
      // The cohortative is *mir wänd* + the infinitive, clause-final as the instruction's is: "mir wänd
      // d Muus ässe" (P10-E13), where German inverts its own verb, "essen wir die Maus".
      const parts = register === 'instruction'
        ...
        : ipn === '1pl'
          ? [word, ...mittelfeld, bareGoverned, plain['base'] ?? '', meansText]
