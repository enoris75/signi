...
export function deImperativeWord(forms: Record<string, string>, pn: DeIPN): string {
  ...
  // The plural command is the plural present (*ässed!*), Zürich having one plural form (P10-E4 D3).
  if (pn === '2pl') return stored ?? forms['2pl_present'] ?? `${base.replace(/e$/, '')}ed`;
  // The cohortative's lead, *mir wänd*: the infinitive follows at the clause end (see `renderClause`).
  return 'mir wänd';
}
...
// The *du* command where the lexeme stores none (every seeded verb does, P10-E4): the infinitive's
// stem, *-e* dropped — *trinke* → *trink*.
