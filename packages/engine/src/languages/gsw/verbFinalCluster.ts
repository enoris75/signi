 ...
 * hinzufügt", A138), keeping the space of one written apart ("die sie rückgängig macht", B40). Empty
 * parts are left for the caller to filter.
 */
export function verbFinalCluster({ v2, tail, finiteLeadsTail, particle = '', particleGap }: VerbComplex): string[] {
  return finiteLeadsTail ? [v2, tail] : [tail, `${particle}${particleGap ?? ''}${v2}`];
}
