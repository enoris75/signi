...
import { complement, el, type Forms, CASA, np } from './it.fixtures.js';
...
describe('complementGloss', () => {
  test('a locative takes its default "in", and the phrase keeps its own determiner', () => {
    expect(complementGloss(gloss({ definiteness: 'all', number: 'plural' }, 'locative'))).toBe('in tutte le case');
  });
  ...
  test('a direction with no relation is the plain goal', () => {
    ...
    expect(complementGloss(gloss({}, 'direction'))).toBe('alla casa');
  });

  test('a relation is the complement\'s own: a place under it, a goal "in" it, fused with the article', () => {
    expect(complementGloss(gloss({}, 'locative', UNDER))).toBe('sotto la casa');
    expect(complementGloss(gloss({}, 'direction', INTO))).toBe('nella casa');
  });
