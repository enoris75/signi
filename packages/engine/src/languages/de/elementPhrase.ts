...
export function elementPhrase(el: ResolvedNounElement, _case: 'nom' | 'acc' | 'dat'): string {
  ...
  // ("hängt von ihr ab", see `objectPrepCase`). An indefinite pronoun keeps its relative clause:
  // "sieht jemanden, der läuft" (A309).
  return coordinate(el, (np) =>
    _case !== 'nom' && np.head.forms['person'] ? `${tonicPronounDe(np.head.forms, _case)}${subordinateClause(np)}` : nounPhrase(np, _case));
}
