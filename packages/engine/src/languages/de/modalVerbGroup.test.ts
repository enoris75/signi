...
describe('modalVerbGroup', () => {
  ...
  test('future and conditional: werden/würde in V2, every modal stacks', () => {
    expect(modalVerbGroup([modal(MUESSEN)], GEHEN, '3sg', 'future', 'neutral')).toEqual({ v2: 'wird', mid: '', tail: 'gehen müssen', zuInfinitive: '' });
    expect(modalVerbGroup([modal(WOLLEN), modal(KOENNEN)], GEHEN, '3sg', 'future', 'neutral'))
      .toEqual({ v2: 'wird', mid: '', tail: 'gehen können wollen', zuInfinitive: '' });
    expect(modalVerbGroup([modal(KOENNEN)], ESSEN, '1sg', 'present', 'neutral', 'conditional'))
      .toEqual({ v2: 'würde', mid: '', tail: 'essen können', zuInfinitive: '' });
  });
  ...
  test('progressive: "gerade" in the Mittelfeld', () => {
    ...
    expect(modalVerbGroup([modal(MUESSEN)], ESSEN, '3sg', 'future', 'progressive')).toEqual({ v2: 'wird', mid: 'gerade', tail: 'essen müssen', zuInfinitive: '' });
  });
  ...
  test('prospective: "im Begriff" with the copula as an infinitive, the zu-infinitive apart', () => {
    ...
    expect(modalVerbGroup([modal(MUESSEN)], GEHEN, '3sg', 'future', 'prospective'))
      .toEqual({ v2: 'wird', mid: 'im Begriff', tail: 'sein müssen', zuInfinitive: 'zu gehen' });
  });
  ...
  test('resultative: Partizip II + the verb’s own auxiliary infinitive', () => {
    ...
    expect(modalVerbGroup([modal(MUESSEN)], GEHEN, '3sg', 'future', 'resultative')).toEqual({ v2: 'wird', mid: '', tail: 'gegangen sein müssen', zuInfinitive: '' });
  });
});
