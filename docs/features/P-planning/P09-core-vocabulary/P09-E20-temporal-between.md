...
**Proposed, not engine output.** The first column is what the *locative* `between` already writes over
a two-day group (see *Today*) with the particle corrected; the French *-ci / -là* needs
[C40](../../../localization/done/C40-french-distal-demonstrative.md)'s `contrastive` on both conjuncts
(D4).
