...
**Status:** planning, unscheduled. Filed 2026-09-23 from P09-E4's and P09-E6's follow-ups.
...
**Proposed, not engine output.** Today the plan cannot be written; forced, it renders the wrong
sentence (see [Today](#today)).
...
- [`ContentClause`](../../../../packages/shared/src/index.ts#L1172) is `subject`, `verbPhrase`,
  `directObject`, `complements`, and its doc comment says it "never becomes a question … it has no
  field for any of them" (L1169–1170). [`contentObject`](../../../../packages/shared/src/index.ts#L1452)
  is typed `ContentClause`.
- **The fields already flow if forced.** `resolvePhrase` resolves the object clause by calling
  itself on it ([L189–191](../../../../packages/engine/src/translator/functions/resolvePhrase.ts#L189)),
  ...
  so the force check at [L91](../../../../packages/engine/src/translator/functions/resolvePhrase.ts#L91)
  ...
  - wh: "the man asks that **what does** the cat eat", "chiede che che cosa mangia il gatto",
    ...
    `render` ([`frenchEngine`](../../../../packages/engine/src/languages/fr/frenchEngine.ts#L42),
    [`germanEngine`](../../../../packages/engine/src/languages/de/germanEngine.ts#L31)) and lose
...
- The complementizers are literals in each `renderClause`: en *that* (L81), it *che* (L73), fr
  `subordinateText('que', …)` (L63), de *dass* (L110), es *que* (L77), pt *que* (L79).
...
- `ContentClause` ([L1172](../../../../packages/shared/src/index.ts#L1172)) gains the four fields
  (D1); its doc comment's "never becomes a question" becomes "only under `contentObject`".
- `contentObject`'s doc comment ([L1452](../../../../packages/shared/src/index.ts#L1452)): D2's
  complementizers, D5.
- `Concept` docs: `content_clause_force` (D4).

## 2. Translator

- [`resolvePhrase.ts`](../../../../packages/engine/src/translator/functions/resolvePhrase.ts#L189):
...
- [`types.ts`](../../../../packages/engine/src/types.ts#L407): `ResolvedPhrase.embedded?` beside
  `contentObject`'s comment.
- D6's guard where `object_prep` is read for a clause object.

## 3. Per-engine rendering

Each `renderClause`'s object-clause branch (en L81, it L73, fr L63, de L110, es L77, pt L79) picks
...
`questionOrder`. Japanese: [`buildClauseSegments`](../../../../packages/engine/src/languages/ja/buildClauseSegments.ts#L136)
