...
[P09-E16](P09-E16-passive-question.md)'s questions can be built: "**what** is eaten by the
...
side is [P09-E16](P09-E16-passive-question.md). Second of four question controls, after
...
Shipped 2026-09-25. `canAsk` keeps every mark in the passive (D1) and refuses every gap in a passive
over a verb with `Concept.prepositionalObject` (D3), derived from the primary lexemes' `object_prep`
by `PREPOSITIONAL_OBJECT_SQL` in `listConcepts` and in `seedConcept`, and pinned in
`definitionText.test.ts` (its `FACTS`, and the sixteen verbs by id). `hasPatient` (D2) keeps the voice
satellite and `rendersPassive` on an empty asked object. The console needed no new word (D5).
Rendered through the real selection → plan → engine path, each from an **empty** asked box:
...
1. **The round-trip walk does not reach a passive question.** At `SEEDS=5000` it reaches about a
   hundred asked plans and no passive one (checked by disabling the D3 gate: the gating block stayed
   green), so it is not the test that would catch a missing D3. `questionGates.test.ts` and
   `askQuestion.test.ts` pin the gate instead, and the console vocabulary's NEED now carries the
   served `prepositionalObject`, so the walk would count it if it got there.
2. **Tests changed**: `questionGates.test.ts` no longer lists the passive among the moods that
