...
one feature from another**. Eleven glosses, no new word, no new construct.)_
...
| MOOD | `whoGloss('FEATURE', 'INDICATE', 'CONDITION')` | a feature that indicates conditions |
...
[B57](../B-needs-seed/B57-ui-nouns-needing-a-word.md) — and none of that blocks this ticket.
...
| MOOD | a feature that indicates conditions | una caratteristica che indica condizioni | une caractéristique qui indique des conditions | ein Merkmal, das Bedingungen bezeichnet | una característica que indica condiciones | 条件を示す特徴 | uma característica que indica condições |
...
All eleven render in all seven, and the German bare-plural object sits before its clause-final verb
in each (*das Zeiten bezeichnet*). Three readings to judge on authoring:

1. **MOOD is the weak one.** Mood is how a clause is *meant* — asserted, ordered, wished — and only
   one of those is a condition. "A feature that indicates conditions" is closer to CONDITIONAL than
   to MOOD, and CONDITION is [A27](A27-grammar-participants-and-clause-types.md)'s own head. If the
   authoring probe agrees, drop MOOD to [C27](../C-needs-engine/C27-grammar-meta-nouns.md) rather
   than ship a gloss that names its neighbour.
...
[B58](../B-needs-seed/B58-tense-and-number-values.md). STATEMENT, ARTICLE and
