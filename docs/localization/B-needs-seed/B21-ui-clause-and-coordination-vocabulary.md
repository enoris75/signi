...
| Add a conjunct / Add another conjunct | [satellites.tsx:787](../../../packages/frontend/src/components/PhraseBuilder/satellites.tsx#L787); "Add another conjunct" again on the last conjunct's ring, [decoratePerimeterControls.ts:44](../../../packages/frontend/src/components/PhraseBuilder/functions/decoratePerimeterControls.ts#L44) | `action.addConjunct` / `action.addAnotherConjunct` | `commandOf('ADD')` + `CONJUNCT indefinite` (+ `adjectives [OTHER]`) |
...
conjunct` → `satellites.test.tsx`, `functions/decoratePerimeterControls.test.ts`; `Remove this conjunct` /
